...
void printVector(const std::vector<int> &myvector) {
  ...
  for (std::vector<int>::const_iterator it = myvector.begin();
       it != myvector.end(); ++it)
    std::cout << ' ' << *it;
  std::cout << '\n';
}
...
int main() {
  ...
  std::cout << "\n\t\t ADD NUMBER WHEN VECTOR IS FULL TEST\n" << std::endl;
  {
    try {
      Span sp = Span(5);
      sp.addNumber(6);
  ...
  // Uncomment and test further if needed . CAUTION : PRINT ALL NUMBERS UNTIL
  // MAX std::cout << "\n\t\t ADDRANGE TEST \n" << std::endl;
  // {
  //     try
  //     {
